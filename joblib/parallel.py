...
def reduce_memmap(a):
    print "Reducing memmap", a.filename
    order = 'F' if a.flags['F_CONTIGUOUS'] else 'C'
    return np.memmap, (a.filename, a.dtype, a.mode, a.offset, a.shape, order)


if multiprocessing is not None and np is not None:
    # Register a dedicated pickler for handling memmap arrays in a
    # multiprocessing environment: new instances pointed to the
    # same backing file are created instead of doing an in-memory
    # serialization of the array data
    print "Registering numpy.memmap pickling support"
    # ForkingPickler is only used for mp.Process args, it is not used
    # when using mp.Pool as the underlying Queue use the regular python
    # pickler...
    # from multiprocessing.forking import ForkingPickler
    # ForkingPickler.register(np.memmap, reduce_memmap)
    import copy_reg
    copy_reg.pickle(np.memmap, reduce_memmap)
